...
- **🎮 交互式拖拽GUI**: 鼠标直接拖动图片、滚轮缩放，真正的所见即所得！
- **🆕 交互式预览**: 带参考线和网格的可视化预览
...
- **滚轮缩放**: 在编辑器区域滚动鼠标滚轮放大/缩小
- **实时显示**: 红色圆圈（裁剪边界）+ 绿色十字（中心参考）

**输入参数：**
- `image`: 输入图片
- `diameter_mm`: 徽章直径
- `dpi`: 分辨率
- `scale`: 缩放（可手动输入或通过滚轮调整）
- `offset_x`: X偏移（可手动输入或通过拖拽调整）
- `offset_y`: Y偏移（可手动输入或通过拖拽调整）
...
**详细教程**: 查看 [INTERACTIVE_GUI_GUIDE.md](INTERACTIVE_GUI_GUIDE.md)

---

### 🆕 交互辅助节点

#### 5. 交互式预览 (InteractivePreviewNode)
